...
function sanitizeSourceKey(s) {
  return normalizeLeadSourceKey(s, LEAD_SOURCE_ISKAHOMES)
}
 ...
 * @param {string | null | undefined} leadSourceContext — only applied when leadSource is iskahomes
 */
export function incrementLeadSourceBreakdown(existing, leadSource, leadSourceContext) {
  const src = sanitizeSourceKey(leadSource)
  const ctx = isIskaHomesSource(src) ? sanitizeContextKey(leadSourceContext) : null

  const b = cloneJson(existing)
  if (b.website) {
    const migrated = b.website
    delete b.website
    if (!b[LEAD_SOURCE_ISKAHOMES]) b[LEAD_SOURCE_ISKAHOMES] = migrated
  }

  if (!b[src]) {
    b[src] = { amount: 0, percentage: 0, ...(isIskaHomesSource(src) ? { context_breakdown: {} } : {}) }
  }
  const node = b[src]
  node.amount = (Number(node.amount) || 0) + 1

  if (isIskaHomesSource(src) && ctx) {
  ...
  for (const key of Object.keys(b)) {
    ...
    if (isIskaHomesSource(key) && e.context_breakdown && typeof e.context_breakdown === 'object') {
