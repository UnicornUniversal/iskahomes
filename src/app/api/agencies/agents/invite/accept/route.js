...
export async function POST(request) {
  try {
    ...
    // Check if user already exists in Auth (admin has no getUserByEmail in auth-js)
    let userId = null
    try {
      const { user: existingUser, error: getUserError } = await findAuthUserByEmail(agent.email)
      if (getUserError) {
