...
const ProfileForm = ({ accountType = 'developer' }) => {
  ...
  const token = accountType === 'agency' ? agencyToken : developerToken
  const apiEndpoint = accountType === 'agency' ? '/api/agencies/profile' : '/api/developers/profile'
        ...
        const normalizedData = {
          ...
          profile_image: (() => {
            ...
            if (typeof data.profile_image === 'string') {
              try {
                const parsed = JSON.parse(data.profile_image)
                return typeof parsed === 'object' && parsed.url ? parsed : data.profile_image
              } catch {
          ...
          cover_image: (() => {
            ...
            if (typeof data.cover_image === 'string') {
              try {
                const parsed = JSON.parse(data.cover_image)
                return typeof parsed === 'object' && parsed.url ? parsed : data.cover_image
              } catch {
  ...
  // Fetch property categories for specialization (only for developers)
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        // Parse profile_image and cover_image if they're JSON strings
        const parsedFormData = { ...formDataFields, locations: normalizedResponseData.locations }
        
        if (parsedFormData.profile_image && typeof parsedFormData.profile_image === 'string') {
          try {
            const parsed = JSON.parse(parsedFormData.profile_image)
            if (typeof parsed === 'object' && parsed.url) {
              parsedFormData.profile_image = parsed
            }
          } catch {
        ...
        if (parsedFormData.cover_image && typeof parsedFormData.cover_image === 'string') {
          try {
            const parsed = JSON.parse(parsedFormData.cover_image)
            if (typeof parsed === 'object' && parsed.url) {
              parsedFormData.cover_image = parsed
            }
          } catch {
            // Keep as string if parsing fails
          }
  ...
  const handleGalleryUpload = (event) => {
    ...
    if (validFiles.length > 0) {
      ...
      // If files were selected but none were valid, show a message
      toast.error('No valid images were added. Please check file sizes and types.', {
        position: 'bottom-center',
        autoClose: 4000,
      })
    }
  ...
  return (
      ...
      <div className="flex-1 md:p-2 md:p-2 md:p-6  relative">
        ...
        <div className="mb-8">
          ...
          <p>Manage your {accountType === 'agency' ? 'agency' : 'developer'} profile and account settings</p>
        </div>
        ...
        {activeTab === 'profile' && (
          <div className=" rounded-2xl shadow-sm border border-gray-100 text-primary_color overflow-hidden">
            {/* Cover Photo */}
            <div className="relative h-80 bg-white/40 rounded-t-2xl overflow-hidden">
              {formData.cover_image ? (
                ...
                <div className="w-full h-full flex items-center justify-center">
                  <div className="text-center text-gray-600">
                    <FiCamera className="w-16 h-16 mx-auto mb-4 text-primary_color" />
                    <p className="text-xl font-medium">Set Cover Image</p>
                    <p className="text-sm opacity-80">Upload a cover photo for your profile</p>
                  </div>
                </div>
              )}
              <label className="absolute top-4 right-4 bg-white/80 backdrop-blur-sm rounded-lg p-2 cursor-pointer shadow-lg hover:bg-white transition-colors z-10">
                ...
                <div className="flex items-end gap-4">
                  <div className="relative">
                    {formData.profile_image ? (
                      ...
                      <div className="w-24 h-24 rounded-full border-4 border-white shadow-lg bg-white/40 flex items-center justify-center">
                        <div className="text-center text-gray-600">
                          <FiUser className="w-8 h-8 mx-auto mb-1 text-primary_color opacity-70" />
                          <p className="text-xs font-medium">Set Profile Photo</p>
                        </div>
                      </div>
                    )}
                    <label className="absolute bottom-0 right-0 bg-white/80 rounded-full p-2 cursor-pointer shadow-lg hover:bg-white transition-colors">
                      <FiCamera className="w-4 h-4 text-primary_color" />
                      <input
                        type="file"
                        accept="image/*"
            ...
            <h2 className="mb-3 ">{formData.name || 'Company Name'}</h2>
                ...
                <div className="flex flex-wrap items-center w-full justify-between gap-4 mb-3">
                  ...
                  {/* Property Specializations - Only for Developers */}
            ...
            <div className="p-2 md:p-2 md:p-6 space-y-8">
              ...
              {/* Section 2: Property Specialization - Only for Developers */}
              {accountType === 'developer' && (
              <div className=" rounded-2xl p-2 md:p-2 md:p-6">
              ...
              {accountType === 'developer' && <hr className="border-white/50 mt-6" />}
