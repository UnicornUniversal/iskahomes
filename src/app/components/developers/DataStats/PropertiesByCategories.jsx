...
const PropertiesByCategories = ({ statsData = null, totalUnits = null }) => {
  const { user } = useAuth()
  
  // Get data from user profile
  const purposes = useMemo(() => {
    if (Array.isArray(statsData) && statsData.length > 0) return statsData
    if (!user?.profile?.property_purposes_stats) return []
    return user.profile.property_purposes_stats
  }, [statsData, user?.profile?.property_purposes_stats])
  ...
  // Prepare chart data
  const pieLabels = purposes.map(purpose => purpose.name || 'Unknown')
  const pieData = purposes.map(purpose => purpose.total_amount || 0)
